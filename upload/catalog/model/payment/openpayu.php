...
class ModelPaymentOpenPayu extends Model {
	public function getMethod($address, $total) {
		...
		$status = true;
		/*
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('openpayu_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
		
		if ($this->config->get('openpayu_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('openpayu_geo_zone_id')) {
			$status = true;
		...
		*/
		...
		if ($status) {
			$method_data = array( 
				...
				'sort_order' => $this->config->get('openpayu_sort_order')
			);
		}
	...
	public function customerupdate($order_id, $data){
		...
		if (!empty($data['shipping_zone'])) {
			...
			$data['shipping_zone'] = 'zone was not provided by PAYU. for further info visit twitter.com/openpayu';
		}
