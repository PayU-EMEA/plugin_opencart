...
class ControllerPaymentOpenPayU extends Controller {
    ...
    protected function loadLibConfig() {
        ...
        OpenPayU_Configuration::setMerchantPosId($this->config->get('openpayu_merchantposid'));
        OpenPayU_Configuration::setPosAuthKey($this->config->get('openpayu_posauthkey'));
        OpenPayU_Configuration::setClientId($this->config->get('openpayu_clientid'));
        OpenPayU_Configuration::setClientSecret($this->config->get('openpayu_clientsecret'));
        OpenPayU_Configuration::setSignatureKey($this->config->get('openpayu_signaturekey'));
        if($this->config->get('openpayu_test')){
    ...
    protected function index() {
        $this->language->load('payment/openpayu');
        $this->load->model('payment/openpayu');

        $this->data['text_testmode'] = $this->language->get('text_testmode');
        $this->data['button_confirm'] = $this->language->get('button_confirm');
        $this->data['testmode'] = $this->config->get('openpayu_test');
        ...
        $grandTotal = 0;
        $cartItems = array();
        $tresc = OpenPayU_Configuration::getEnvironment()."\n".OpenPayU_Configuration::getPosAuthKey()."\n";
        ...
        $order = array (	'MerchantPosId' => OpenPayU_Configuration::getMerchantPosId(),
            ...
            'OrderUrl' => $this->url->link('payment/openpayu/callback') . '?order=' . $this->session->data['order_id'],
        ...
        $OCReq = array (	'ReqId' =>  md5(rand()),
            ...
            'NotifyUrl' => $this->url->link('payment/openpayu/ordernotify'), // url where payu service will send notification with order processing status changes
            'OrderCancelUrl' => $this->url->link('payment/openpayu/paymentcancel'),
            'OrderCompleteUrl' => $this->url->link('payment/openpayu/paymentsuccess'),
            'Order' => $order);
        if ($orderType == 'MATERIAL') {
            ...
            $OCReq['ShippingCost'] = array(
                ...
                'ShippingCostsUpdateUrl' => $this->url->link('payment/openpayu/shipping')
            );
        ...
        //Setting template :)
        $this->data['beforesummary'] = $this->url->link('payment/openpayu/beforesummary','','SSL');
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/openpayu.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/openpayu.tpl';
        } else {
            $this->template = 'default/template/payment/openpayu.tpl';
        }
        $this->render();
    }
...
//Before summary redirection
    public function beforesummary() {
        $this->loadLibConfig();
        $result = OpenPayU_OAuth::accessTokenByCode($_GET['code'], $this->url->link('payment/openpayu/beforesummary','','SSL'));
...
//Shipping update
    public function shipping() {
        ...
        $this->loadLibConfig();
        $xml = htmlspecialchars_decode($_REQUEST['DOCUMENT']);
...
//Notification
    public function ordernotify(){
        ...
        $this->loadLibConfig();
        $doc = htmlspecialchars_decode($_REQUEST['DOCUMENT']);
        ...
        if (!empty($paymentStatus)){
            switch($paymentStatus){
                case "PAYMENT_STATUS_NEW" :
                    return $this->config->get('openpayu_new_status');
                case "PAYMENT_STATUS_CANCEL" :
                    return $this->config->get('openpayu_cancelled_status');
                case "PAYMENT_STATUS_REJECT" :
                    return $this->config->get('openpayu_reject_status');
                case "PAYMENT_STATUS_INIT" :
                    return $this->config->get('openpayu_pending_status');
                case "PAYMENT_STATUS_SENT" :
                    return $this->config->get('openpayu_sent_status');
                case "PAYMENT_STATUS_NOAUTH" :
                    return $this->config->get('openpayu_failed_status');
                case "PAYMENT_STATUS_REJECT_DONE" :
                    return $this->config->get('openpayu_returned_status');
                case "PAYMENT_STATUS_END" :
                    return $this->config->get('openpayu_complete_status');
...
//Customer data update from the data sent by PayU
    public function updatecustomerdata($order_id ,$customerdata){
        ...
        $this->model_payment_openpayu->customerupdate($order_id, $update_data);
    }
...
//express checkout - OK
    public function expresscheckout(){
        ...
        if($result->getSuccess()){
            $beforesummary = $this->url->link('payment/openpayu/beforesummary','','SSL');
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/openpayu_redirect.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/payment/openpayu_redirect.tpl';
            } else {
                $this->template = 'default/template/payment/openpayu_redirect.tpl';
            }
            $this->response->setOutput($this->render());

        }else{
            header("Location: ".$this->url->link('checkout/cart'));
            $error_info = $result->getStatus();
        }
    }
...
//test functions
    public function addscript(){
        echo "Dodaję skrypt \n";
        $script = '/catalog/view/javascript/openpayu.js';
        echo $script;
...
//collecting data for expresscheckout
    public function collectData(){
        ...
        $allData['payment_method']						= "openpayu";
        $allData['payment_code']						= "openpayu";
